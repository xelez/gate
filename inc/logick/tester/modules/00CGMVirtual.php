<?php
  ...
  if ($__CGMVirtual_Included__ != '##CGMVirtual_Included##') {
    ...
    class CGMVirtual extends CVirtual {
      ...
      var $module;
      function CGMVirtual ($gw) {
        $this->SetClassName ('CGMVirtual');
        $this->gateway = $gw;
      }
      ...
      function Template ($tpl, $vars = array ()) {
        return tpl ('front/tester/modules/'.$this->GetModuleName ().
                    '/'.$tpl, $vars);
      }
      ...
      function GetAllowed ($action) {
        return $this->gateway->GetAllowed ($action);
      }
      ...
      function GetUsersAtContest ($contest_id = -1) {
        ...
        return arr_from_query ('SELECT `u`.`id`, `u`.`name` '.
                               'FROM `user` AS `u`, `usergroup` AS `ug`, '.
                               '`tester_contestgroup` AS `tcg` '.
                               'WHERE (`tcg`.`contest_id`='.$contest_id.') '.
                               'AND (`ug`.`group_id`=`tcg`.`group_id`) AND '.
                               '(`u`.`id`=`ug`.`user_id`) '.
      ...
      function GetJudgesAtContest ($contest_id = -1) {
        ...
        return arr_from_query ('SELECT `u`.`id`, `u`.`name` '.
                               ...
                               'WHERE (`tjg`.`contest_id`='.$contest_id.') '.
                               'AND (`ug`.`group_id`=`tjg`.`group_id`) '.
                               'AND (`u`.`id`=`ug`.`user_id`) '.
                               'GROUP BY `u`.`id` ORDER BY `u`.`name`'
                               );
      }
