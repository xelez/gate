<?php
  ...
  if ($_image_validator_ != '###imageValidator_Included###') {
    ...
    function validate_image_cmpiterator ($val, $key, $field, $type, $dimension) {
      ...
      if ($r == 'NOTCOMPILES') {
        return $field.' изображения не '.$eq.' '.
          parseint ($key).' '.$dimension.'.';
      }

      if ($r == 'GREATER') {
        return $field.' изображения превосходит '.
          parseint ($key).' '.$dimension.'.';
      }
    ...
    function validate_image ($data, $size = '', $hlimit = '', $vlimit = '') {
      ...
      if ($res) {
        ...
        $w = $image_size[0];
        $h = $image_size[1];
        $r = validate_image_cmpiterator ($data['size'], $size, 'Размер',
                                         1, 'байт');
        if ($r != '') {
          return $r;
        }

        $r = validate_image_cmpiterator ($w, $hlimit, 'Ширина', 0,
                                         'пикселей');
        if ($r != '') {
          return $r;
        }
