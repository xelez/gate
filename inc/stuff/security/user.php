<?php
  ...
  if ($IFACE != "SPAWNING NEW IFACE") {
    ...
    function user_check_fields ($login, $name, $passwd, $email,
                                $check_login = true, $skipId = -1) {
      // Get settings
    ...
    function user_create ($login, $name, $passwd, $email,
                          $authorized = false, $access = 1, $groups = array ()) {
      // Check da values
      ...
      db_insert ('user', array ('name' => '"'.
                                    htmlspecialchars (addslashes ($name)).'"',
                                'login' => '"'.addslashes ($login).'"',
                                'password' => 'MD5("'.
                                    addslashes (user_password_hash ($login, $passwd)).
                                    '")',
                                'access' => $access,
    ...
    function user_create_received ($authorized = true) {
      ...
      if (user_create ($login, $name, $passwd, $email, $authorized,
                       $acc, $groups->GetItemsUsed ())) {
        $_POST = array ();
        return true;
      }
    ...
    function user_update ($id, $name, $email, $access,
                          $groups = array (), $passwd = '') {
